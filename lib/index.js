...
const checkLogging = require('./steps/checkLogging')
...
module.exports = async (customCwd) => {
  ...
  try {
    ...
    if (jsreportInstance._fnAfterConfigLoaded != null) {
      const originalAfterConfig = jsreportInstance._fnAfterConfigLoaded

      jsreportInstance.afterConfigLoaded((reporter) => {
        originalConfig = Object.assign({}, reporter.options)
        originalAfterConfig.call(jsreportInstance, reporter)
      })
    }
    ...
    await checkLogging(originalConfig)
    ...
    // TODO: THINGS TO DO WHEN RELEASING THE CLI:
    // - change "jsreport/jsreport#2.0.0-beta" in installNewDeps to use real v2 package
    // - probably add a link to full list of jsreport v2 changes at the end or start of CLI.
    ...
    response = await installNewDeps('jsreport/jsreport#2.0.0-beta', packagesToInstall)
